...
**Review Latency & Grounding (PLAN-REVIEW-LATENCY.md):**
- **Tier 0 (Transport):** HTTP-first role transport configured via `"roles": {"transport": "http"}` with loud degradation events (`role_transport_degraded`) recording reason (`no_api_key`, `no_base_url`, `resolve_failed`). Adapter construction hoisted out of retry loop in `BackendRoleProvider.complete_json`. Reviewer fan-out parallelized across headings with cancel-on-regenerate. Default `review_sample_rate` set to `0.05`, `max_parallel` derived floor raised to 8–16, reviewer timeout tightened to 45s.
- **Tier 1 (Scope & Triage):** Hard gate graduation for unambiguous checks (`latex_balanced`, `headers:std`, `refs_resolve`); deterministic pre-filter skips reviewer model call entirely when all gates pass and all judgments are gate-covered. Fast two-stage triage schema (`TRIAGE_SCHEMA`) runs cheap `suspect` filter. Delta reviewing on retries via cached per-section verdict digests. Cross-leaf defects (`coverage_gap`, `duplicate_content`, `register_drift`, `terminology_drift`) stripped from per-node reviewer in favor of windowed document review.
- **Tier 2 (Grounding & Closed-World):** Ground truth passed into reviewer prompt (`contract_text`, `declared_inputs`) and verdict digest. Rubric items classified `closed` vs `open` in `NodeTemplate` with open items excluded from model review. Factual assertions route to writer-emitted `claims.jsonl` checked deterministically via `refs_resolve`. Reviewer precision benchmarked across 42 artifacts (6 defect classes including `uncited_claim` and `contradicts_contract`) with `mean_wall_clock_per_call` budget tracking.
